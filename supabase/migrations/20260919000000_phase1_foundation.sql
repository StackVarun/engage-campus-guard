...
-- Faculty accounts are intentionally provisioned outside public registration.
-- After creating an auth user in Supabase, provision it with privileged SQL:
--   insert into public.faculty_profiles (user_id, full_name) values ('<auth-user-id>', '<name>');
--   update public.user_accounts set role = 'FACULTY' where id = '<auth-user-id>';
